...
class Amplifier():
	...
	Class representing a generic Rare-Earth doped fiber amplifier
		...
		* nbrSections: Number of longitudinale section
	...
	def get_aseSpectrum(self):
		...
		return [ase_forward, ase_backward]
	...
	def simulate(self, direction=1, backwardOutput=False):
		...
		self.error = abs(self.initNoise - solution[:,2*self.nbrSignal+2*self.nbrPump+self.nbrAse:2*self.nbrSignal+2*self.nbrPump+2*self.nbrAse].T[0,-1])
	...
	def run(self, errorTol, nbrItrMax, errorOutput = False, verbose=False):
		...
		for i in arange(nbrItrMax):
			...
			self.simulate(backwardOutput=True)
			if verbose:
				print self.P_p_b[:,-1]
