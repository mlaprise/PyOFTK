...
int IsComplex(COMPLEX*, int);
double nlPhaseShift(COMPLEX* field_amp, double gamma, double dz, int nb_points);
void cscale(COMPLEX*,COMPLEX*,COMPLEX*,COMPLEX*,REAL,int);
void rotate_coord(COMPLEX*,COMPLEX*,COMPLEX*,COMPLEX*,REAL,REAL,int);
void compute_w(REAL*,REAL,int);
void compute_hahb(COMPLEX*,COMPLEX*,int,double*,int,double*,int,
	double*,int,double*,REAL*,REAL,int);
...
void inv_rotate_coord(COMPLEX*,COMPLEX*,COMPLEX*,COMPLEX*,REAL,REAL,int);
...
void testNumpy(COMPLEX* a, int na, COMPLEX *b, int nb, COMPLEX* AplusB, int nAplusB)
{
	...
	for(i = 0; i < na; i++)
	{
		AplusB[i][0] = a[i][0] + b[i][0];
		AplusB[i][1] = a[i][1] + b[i][1];
	}
...
{ 
  ...
  if (wisfile) {
    //printf("Exporting FFTW wisdom (file = %s).\n", WISFILENAME);
...
{ 
  ...
  if (wisfile) {
	...
	if (!IMPORT_WISDOM(wisfile)) {
      fclose(wisfile);
	  // Error Msg
      printf("could not import wisdom.");
    }
    else
	  fclose(wisfile);
  }
...
int IsComplex(COMPLEX* a, int lenght)
{
	int flag = 0;
	int i;
...
double nlPhaseShift(COMPLEX* field_amp, double gamma, double dz, int nb_points)
{
	...
	double phiNL;
	double *field_int;
	field_int = (double*) fftw_malloc(sizeof(double) * nb_points);	
	for(i = 0; i < nb_points; i++)
	{
		field_int[i] = field_amp[i][0]*field_amp[i][0] + field_amp[i][1]*field_amp[i][1];
		if(field_int[i] > peakValue)
		{
			peakValue = field_int[i];
		}
	}

	phiNL = gamma*dz*peakValue;
	fftw_free(field_int);
	return phiNL;
}
 ...
 */
void rotate_coord(COMPLEX* u0a, COMPLEX* u0b,COMPLEX* ux,COMPLEX* uy,
...
{ 
  ...
  int jj;
  if (IsComplex(ux,nt) && IsComplex(uy,nt))
    for(jj = 0; jj < nt; jj++) {
      u0a[jj][0] = cc*ux[jj][0] + ss*ux[jj][1] + 
                   sc*uy[jj][0] - cs*uy[jj][1];
      u0a[jj][1] = cc*ux[jj][1] - ss*ux[jj][0] + 
                   sc*uy[jj][1] + cs*uy[jj][0];
      u0b[jj][0] = -sc*ux[jj][0] - cs*ux[jj][1] + 
                   cc*uy[jj][0] - ss*uy[jj][1];
      u0b[jj][1] = -sc*ux[jj][1] + cs*ux[jj][0] + 
                   cc*uy[jj][1] + ss*uy[jj][0];
    }
  else if (IsComplex(ux,nt))
    for(jj = 0; jj < nt; jj++) {
      u0a[jj][0] = cc*ux[jj][0] + ss*ux[jj][1] + 
                   sc*uy[jj][0];
      u0a[jj][1] = cc*ux[jj][1] - ss*ux[jj][0] + 
 ...
 */
void compute_hahb(COMPLEX* ha,COMPLEX* hb,int nalphaa,double* alphaa,
                  int nalphab ,double* alphab,int nbetaa, double* betaa, int nbetab, double* betab,
 ...
 */
void inv_rotate_coord(COMPLEX* u1x, COMPLEX* u1y,COMPLEX* u1a,COMPLEX* u1b,
                      REAL chi, REAL psi, int nt) 
{ 
  ...
  int jj;
  for(jj = 0; jj < nt; jj++) {
    u1x[jj][0] = cc*u1a[jj][0] - ss*u1a[jj][1] -
                       sc*u1b[jj][0] + cs*u1b[jj][1];
    u1x[jj][1] = cc*u1a[jj][1] + ss*u1a[jj][0] -
                       sc*u1b[jj][1] - cs*u1b[jj][0];
    u1y[jj][0] = sc*u1a[jj][0] + cs*u1a[jj][1] +
                       cc*u1b[jj][0] + ss*u1b[jj][1];
    u1y[jj][1] = sc*u1a[jj][1] - cs*u1a[jj][0] +
...
{ 
  ...
  int nt = nu0x;            /* number of fft points */
  
  REAL* w;           /* vector of angular frequencies */
  double *phiNL;
  double totalNLPhaseShift = 0.0;
  ...
  int elliptical = 0;       /* if elliptical method, then != 0 */
  int nalpha = nalphaa;		/* alphaa and alphab have the same lenght */
  int nbeta = nbetaa;  
  ...
  if (nrhs < 10) 
    // Error Msg
    printf("Not enough input arguments provided.");
  if (nlhs > 2)
    // Error Msg
...
/*
  ...
  if (nrhs > 11) { //default method is elliptical
    if (mxGetString(prhs[11],methodstr,11)) // fail
	  // Error Msg
      printf("incorrect method: elliptical or ciruclar only");
    else { // success
  ...
  */
  /* allocate memory */
  u0a = (COMPLEX*) fftw_malloc(sizeof(COMPLEX)*nt);
  u0b = (COMPLEX*) fftw_malloc(sizeof(COMPLEX)*nt);
  uafft = (COMPLEX*) fftw_malloc(sizeof(COMPLEX)*nt);
  ubfft = (COMPLEX*) fftw_malloc(sizeof(COMPLEX)*nt);
  uahalf = (COMPLEX*) fftw_malloc(sizeof(COMPLEX)*nt);
  ubhalf = (COMPLEX*) fftw_malloc(sizeof(COMPLEX)*nt);
  uva = (COMPLEX*) fftw_malloc(sizeof(COMPLEX)*nt);
  uvb = (COMPLEX*) fftw_malloc(sizeof(COMPLEX)*nt);
  u1a = (COMPLEX*) fftw_malloc(sizeof(COMPLEX)*nt);
  u1b = (COMPLEX*) fftw_malloc(sizeof(COMPLEX)*nt);
  ha = (COMPLEX*) fftw_malloc(sizeof(COMPLEX)*nt);
  hb = (COMPLEX*) fftw_malloc(sizeof(COMPLEX)*nt);
  h11 = (COMPLEX*) fftw_malloc(sizeof(COMPLEX)*nt);
  h12 = (COMPLEX*) fftw_malloc(sizeof(COMPLEX)*nt);
  h21 = (COMPLEX*) fftw_malloc(sizeof(COMPLEX)*nt);
  h22 = (COMPLEX*) fftw_malloc(sizeof(COMPLEX)*nt);
  w = (REAL*)fftw_malloc(sizeof(REAL)*nt);
  ...
  compute_hahb(ha,hb,nalpha,alphaa,nalpha,alphab,nbeta,betaa,nbeta,betab,w,dz,nt);

  //printf("Performing split-step iterations ... ");
  ...
  if (elliptical) { /* Elliptical Method */
    ...
    rotate_coord(u0a,u0b,u0x,u0y,chi,psi,nt);
     ...
     *        ( cos(psi)*cos(chi) - j*sin(psi)*sin(chi))*u1b;
     */
    inv_rotate_coord(u1x,u1y,u1a,u1b,chi,psi,nt);
     ...
     *   u0a = (1/sqrt(2)).*(u0x + j*u0y);
     *   u0b = (1/sqrt(2)).*(j*u0x + u0y); */
    rotate_coord(u0a,u0b,u0x,u0y,pi/4,0,nt);
    ...
    for(iz=1; iz <= nz; iz++)
    {
      ...
      cscale(u0a,u1a,u0b,u1b,1.0,nt);
	  // double nlPhaseShift(COMPLEX* field_amp, double gamma, double dz, int nb_points)
	  totalNLPhaseShift = totalNLPhaseShift + nlPhaseShift(u1a, gamma, dz, nt) + nlPhaseShift(u1b, gamma, dz, nt);

    } /* end step loop */
	
	usefulStuff[0] = totalNLPhaseShift;
	//printf("phiNL: %f  pi ", totalNLPhaseShift/pi);    

    /* Rotate back to orignal x-y basis
     *   u1x = (1/sqrt(2)).*(u1a-j*u1b) ;
     *   u1y = (1/sqrt(2)).*(-j*u1a+u1b) ; */
    inv_rotate_coord(u1x,u1y,u1a,u1b,pi/4,0,nt);
